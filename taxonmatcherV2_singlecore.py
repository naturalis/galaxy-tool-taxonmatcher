...
def write_header():
    if args.reference == "nsr":
        with open(args.output, "a") as output:
            output.write("\t".join(["#Input","#MatchType","#Synonym name","#Accepted name","#Taxon rank", "#Kingdom","#Phylum","#Class","#Order","#Family","#Genus","#Metadata","#Input read"]) + "\n")
...
def taxonmatch_nsr_blast():
    ...
    results = []
    for x in taxonomyHits:
        output = match_blast(speciesList, genusList, x)
...
def taxonmatch_nsr_nameslist():
    ...
    results = []
    for x in taxonomyHits:
        output = match_nameslist(speciesList, genusList, x)
        with open(args.output, "a") as outputFile:
            outputFile.write("\t".join(output)+"\n")
